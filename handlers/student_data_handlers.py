...
# async def education_degree_chosen(update: Update, context: CallbackContext, data_loader, ui_builder, user_data_store) -> int:
#     query = update.callback_query
#     await query.answer()
#     user_id = update.effective_user.id
#     degree = query.data.split('_')[1]
...
#     user_data_store[user_id]['education_degree'] = degree  # Зберігаємо освітній ступінь безпосередньо

#     if role == ST_ROLE:
#         faculty = user_data_store[user_id].get('faculty')
#         departments = data_loader.get_departments(faculty) # Отримуємо спеціальності для обраного факультету
#         reply_markup = ui_builder.build_keyboard(departments, 'speciality_')
...
# async def speciality_chosen(update: Update, context: CallbackContext, data_loader, ui_builder, user_data_store) -> int:
#     query = update.callback_query
#     await query.answer()
#     user_id = update.effective_user.id
#     speciality = query.data.split('_')[1]
...
#     user_data_store[user_id]['speciality'] = speciality  # Зберігаємо спеціальність безпосередньо

#     if role == ST_ROLE:
#         courses = ["1", "2", "3", "4", "5", "6"] # Або отримайте з data_loader
#         reply_markup = ui_builder.build_keyboard(courses, 'course_')
#         current_selection = ui_builder.build_selection_text(user_data_store[user_id])
#         message_text = f"{current_selection}\nБудь ласка, оберіть ваш курс:"
#         await query.edit_message_text(text=message_text, reply_markup=reply_markup)
...
# async def course_chosen(update: Update, context: CallbackContext, data_loader, ui_builder, user_data_store) -> int:
#     query = update.callback_query
#     await query.answer()
#     user_id = update.effective_user.id
#     course = query.data.split('_')[1]
...
#     user_data_store[user_id]['course'] = course  # Зберігаємо курс безпосередньо

#     if role == ST_ROLE:
#         # Тепер можна переходити до вибору документа
#         popular_docs = data_loader.get_popular_documents(role)
#         all_docs = data_loader.get_documents(role)
#         keyboard = [[InlineKeyboardButton(doc, callback_data=f'document_{doc}')] for doc in popular_docs]
#         if all_docs and popular_docs != all_docs:
