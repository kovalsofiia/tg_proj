...
# async def get_phone_number(update: Update, context: CallbackContext, data_loader, ui_builder, user_data_store) -> int:
#     user_id = update.effective_user.id
#     contact = update.message.contact
#     phone_number = None
...
#     if phone_number:
#         user_data_store[user_id]['phone_number'] = phone_number  # Зберігаємо номер телефону безпосередньо
#         faculties = data_loader.get_faculties()
#         reply_markup = ui_builder.build_keyboard(faculties, 'faculty_')
...
# async def phone_number_received(update: Update, context: CallbackContext, data_loader, ui_builder, user_data_store) -> int:
#     return await get_phone_number(update, context, data_loader, ui_builder, user_data_store)